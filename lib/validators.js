...
/**
 * Validate bundler detection
 */
export function isValidBundler(value, type) {
  if (!value || !type) {
    return false;
  }

  // If we have a version number, it's valid
  if (/\d+\.\d+\.\d+/.test(value)) {
    return true;
  }

  // Runtime signatures without version are also valid
  ...
  if (runtimeSignatures.some(sig => value.includes(sig))) {
    return true;
  }
